...
> **Verified against code (Phase-1/3 de-risk):** two invariants this design leans on are confirmed present today. (a) **Ordinal determinism** — `tool_callback` stamps the id *synchronously before returning the promise* (`callbacks.rs:61-62`, `:71`) with no host round-trip in between, and the isolate is single-threaded (`mod.rs:177`), so `Promise.all([a(),b(),c()])` deterministically yields `tool-1/2/3` in source order regardless of which host response returns first; resolution is by id lookup, never arrival order (`module_loader.rs:66-77`). `agent_callback` inherits this exactly. (b) The delete list is confirmed to currently omit `Date`/`Math`/`WeakRef`/`FinalizationRegistry` (`globals.rs:16-19` deletes only `console`/`Atomics`/`SharedArrayBuffer`/`WebAssembly`) — the harden above closes it.
...
- **R1 — Resume determinism is load-bearing and fragile (Critical).** Prefix replay is meaningless if the script is nondeterministic. `Date.now`/`Math.random`/`WeakRef`/`FinalizationRegistry` are *not* currently disabled (`globals.rs:16-19`); shipping the journal without the harden makes resume unsound. **A code deep-dive surfaced a second source the original harden missed: wall-clock `setTimeout` interleaving** (`timers.rs:39-42` vs the command loop at `mod.rs:234-256`) — timer-vs-agent ordering is nondeterministic and unfixed by the `Date`/`Math` shims. Mitigation: Phase 3 depends on 3a; the 3a harden now **also removes `setTimeout`/`setInterval`** from the workflow isolate (§7); key by `(prompt,opts)` content hash + invocation ordinal (not wall-clock); ship resume opt-in/experimental; version the key algorithm. *The core replay invariant (synchronous source-order ordinal assignment, id-keyed resolution) is confirmed already implemented (`callbacks.rs:61-62`, `module_loader.rs:66-77`), so the residual risk is narrowed to completing the harden.*
- **R2 — ~~Blocking a JS promise on a long-running subagent in a single isolate~~ (RESOLVED — was High).** A code deep-dive confirms the existing bridge already supports many concurrently-suspended host awaits with **no structural change**: `pending_tool_calls` is an id-keyed map (`mod.rs:148`), resolution is out-of-order by id (`module_loader.rs:66-101`), and `run_cell` spawns **one independent tokio task per call** into an unbounded `JoinSet` (`cell_actor/mod.rs:399-417`, `cell_actor/callbacks.rs:52-76`) over unbounded channels with no serializing mutex — demonstrated by the existing test `execute_to_pending_identifies_tool_calls_in_paused_frontier` (`service_tests.rs:291-329`, two concurrent `tool-1`/`tool-2` in the paused frontier). A 16-way `Promise.all` of `agent()` calls is N-safe as-is; the only concurrency ceiling is the one the workflow scheduler *deliberately* imposes (§5 semaphore) plus each call's consume-to-`TurnComplete` loop (§6). Residual: validate the consume loop itself doesn't hold state that serializes — prototype in Phase 1.
...
1. ~~**Isolate concurrency**: does `cell_actor`/`runtime.rs` support many simultaneously-suspended `agent()` promises?~~ **RESOLVED — yes, as-is.** Id-keyed `pending_tool_calls`, out-of-order id resolution, one tokio task per call in an unbounded `JoinSet`, unbounded channels, no serializing mutex (see R2). No change required to the bridge; the only remaining Phase-1 check is the per-call consume-to-`TurnComplete` loop (open question 9).
