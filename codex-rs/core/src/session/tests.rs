...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let session = Session {
        ...
        tx_event,
        event_observers: tokio::sync::broadcast::channel(16).0,
...
async fn try_start_turn_if_idle_rejects_active_turn_without_injecting() {
    ...
    let item = user_message(&"x".repeat(crate::context::MAX_WORKFLOW_CHILD_CONTEXT_ITEM_BYTES + 1));
    let err = sess
...
#[tokio::test]
async fn workflow_child_rejects_extension_initiated_idle_turn() {
    let (sess, _tc, _rx) = make_workflow_session_and_context_with_rx().await;
    let item = user_message("must remain supervisor-owned");

    let err = sess
        .try_start_turn_if_idle(vec![item.clone()])
        .await
        .expect_err("workflow child must reject extension idle work");

    assert_eq!(
        TryStartTurnIfIdleRejectionReason::WorkflowManaged,
        err.reason()
    );
    assert_eq!(vec![item], err.into_input());
