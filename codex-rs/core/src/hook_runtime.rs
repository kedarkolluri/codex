...
pub(crate) async fn record_pending_input(
    ...
    let prompt_rejected = match pending_input {
    ...
    if prompt_rejected {
        let err = CodexErr::InvalidRequest(
            "workflow child prompt exceeds finalized model-context limits".to_string(),
        );
        ...
        sess.send_event(
            ...
            EventMsg::Error(ErrorEvent {
                message: "workflow child prompt exceeds finalized model-context limits".to_string(),
...
pub(crate) async fn record_additional_contexts(
    ...
    if sess.is_workflow_managed_agent().await {
        developer_messages = crate::context::bound_workflow_child_context_items(developer_messages);
    }
...
mod tests {
    ...
    async fn workflow_pending_response_context_is_split_and_bounded_before_recording() {
        ...
        let item = ResponseItem::Message {
            ...
            content: vec![
                ContentItem::InputText { text: oversized },
