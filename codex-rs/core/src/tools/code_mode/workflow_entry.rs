...
pub(crate) async fn start_saved_workflow_for_turn(
    ...
    ensure_workflow_owner_entry(&exec)?;
...
fn ensure_workflow_owner_entry(exec: &ExecContext) -> Result<(), FunctionCallError> {
    if crate::agent::control::is_workflow_managed_thread_source(
        exec.turn.turn_metadata_state.thread_source(),
    ) {
        return Err(FunctionCallError::RespondToModel(
            "workflow_run is unavailable in workflow-managed threads".to_string(),
        ));
    }
    Ok(())
}
