...
async fn multi_agent_v2_prompt_bounds_are_inactive_without_workflow() -> std::io::Result<()> {
    ...
    let root_agent_usage_hint_text =
        "r".repeat(multi_agent_v2_bounds::PROMPT_FIELD_MAX_BYTES);
    let subagent_usage_hint_text =
        "s".repeat(multi_agent_v2_bounds::PROMPT_FIELD_MAX_BYTES);
    std::fs::write(
        codex_home.path().join(CONFIG_TOML_FILE),
        format!(
            r#"[features]
...
#[tokio::test]
async fn workflow_rejects_multi_agent_v2_prompt_field_over_utf8_byte_limit()
-> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let usage_hint_text = "é".repeat(multi_agent_v2_bounds::PROMPT_FIELD_MAX_BYTES / 2 + 1);
...
async fn workflow_rejects_combined_multi_agent_v2_prompt_payload() -> std::io::Result<()> {
    ...
    let root_agent_usage_hint_text =
        "r".repeat(multi_agent_v2_bounds::PROMPT_FIELD_MAX_BYTES - 1);
