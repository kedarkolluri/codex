...
pub(crate) fn bound_workflow_child_tool_specs(tools: Vec<ToolSpec>) -> Vec<ToolSpec> {
    bound_workflow_child_tool_specs_with_aggregate(tools, MAX_WORKFLOW_CHILD_TOOL_SPECS_BYTES)
}
        ...
        match tool {
            ToolSpec::Namespace(namespace) => {
                ...
                for namespace_tool in tools {
                    ...
                    if !tool_array_with_candidate_fits(&bounded, &candidate, aggregate_bytes) {
                        continue;
                    }
...
pub(crate) fn bound_workflow_child_output_items(
    ...
    for item in items {
        ...
        if output_items.len() >= MAX_WORKFLOW_CHILD_OUTPUT_ITEMS {
            ...
            remove_matching_call_from_batch(&evicted_output, &mut bounded, &mut output_positions);
        }

        let item = bound_workflow_child_output_item(item);
        if serialized_len(&item).is_none_or(|bytes| bytes > MAX_WORKFLOW_CHILD_OUTPUT_PAYLOAD_BYTES)
        {
...
fn tool_array_with_candidate_fits(
    ...
    if serialized_len(candidate).is_none_or(|bytes| bytes > MAX_WORKFLOW_CHILD_TOOL_SPEC_BYTES) {
        return false;
    }
...
fn responses_lite_additional_tools_fits(tools: &[ToolSpec]) -> bool {
    ...
    serialized_len(&item).is_some_and(|bytes| bytes <= MAX_WORKFLOW_CHILD_OUTPUT_PAYLOAD_BYTES)
}
...
fn embedded_payload_budget<T: Serialize>(empty_item: &ResponseItem, empty_payload: &T) -> usize {
    ...
    MAX_WORKFLOW_CHILD_OUTPUT_PAYLOAD_BYTES.saturating_sub(item_bytes.saturating_sub(payload_bytes))
}
...
fn bound_function_output_payload(
    ...
    let body = match body {
        FunctionCallOutputBody::Text(text) => {
            let text = bound_text_for_serialization(text, max_serialized_bytes, |candidate| {
        ...
        FunctionCallOutputBody::ContentItems(items) => {
            ...
            for item in items {
                ...
                if serialized_len(&candidate).is_none_or(|bytes| bytes > max_serialized_bytes) {
                    continue;
                }
...
fn bound_tool_values(
    ...
    for tool in tools {
        ...
        if serialized_len(&tool).is_none_or(|bytes| bytes > MAX_WORKFLOW_CHILD_TOOL_SPEC_BYTES) {
            continue;
        }
...
pub(super) fn omitted_tool_output(item: ResponseItem) -> ResponseItem {
    match item {
...
fn bound_text_for_serialization(
    ...
    let max_bytes = if serialized_len_for(&text).is_some_and(|bytes| bytes <= max_serialized_bytes)
    {
        return text;
    } else {
    ...
    loop {
        ...
        if serialized_len_for(&candidate).is_some_and(|bytes| bytes <= max_serialized_bytes) {
            return candidate;
        }
        if retained_bytes == 0 {
            return String::new();
