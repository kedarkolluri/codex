...
fn exact_byte_cap_is_preserved_and_cap_plus_one_is_truncated() {
    ...
    let over = format!(
        "OPEN{}CLOSE",
        "é".repeat(MAX_WORKFLOW_CHILD_CONTEXT_ITEM_BYTES)
    );
...
fn split_clears_untrusted_envelope_and_preserves_phase() {
    let original_id =
        ResponseItemId::from_server("x".repeat(MAX_WORKFLOW_CHILD_CONTEXT_ITEM_BYTES));
...
fn lowers_unknown_roles_and_omits_stale_multi_agent_guidance() {
    let stale_mode = format!("{MULTI_AGENT_MODE_OPEN_TAG}spawn agents{MULTI_AGENT_MODE_CLOSE_TAG}");
    let stale_hint =
