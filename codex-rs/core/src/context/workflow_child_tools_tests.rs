...
fn tool_specs_drop_oversized_entries_without_reordering_safe_entries() {
    ...
    let bounded =
        bound_workflow_child_tool_specs(vec![safe_before.clone(), oversized, safe_after.clone()]);
...
fn namespace_drops_only_unsafe_nested_specs_and_preserves_safe_order() {
    let namespace = ToolSpec::Namespace(ResponsesApiNamespace {
        ...
        tools: vec![
            ...
            namespace_function("unsafe", "x".repeat(MAX_WORKFLOW_CHILD_TOOL_SPEC_BYTES)),
            ...
            body: FunctionCallOutputBody::ContentItems(
                vec![
                    ...
                    FunctionCallOutputContentItem::EncryptedContent {
                        encrypted_content: "E".repeat(MAX_WORKFLOW_CHILD_OUTPUT_CONTENT_ITEM_BYTES),
                    },
                    FunctionCallOutputContentItem::InputText {
                ...
                .into_iter()
                .chain(
                    (0..MAX_WORKFLOW_CHILD_OUTPUT_CONTENT_ITEMS + 8).map(|index| {
                        FunctionCallOutputContentItem::InputText {
...
fn tool_search_output_bounds_each_tool_count_and_aggregate() {
    let oversized =
        json!({"name": "oversized", "description": "x".repeat(MAX_WORKFLOW_CHILD_TOOL_SPEC_BYTES)});
    let tools = std::iter::once(json!({"name": "safe-before"}))
        .chain(std::iter::once(oversized))
        .chain((0..MAX_WORKFLOW_CHILD_OUTPUT_CONTENT_ITEMS + 8).map(
            |index| json!({"name": format!("tool-{index:03}"), "description": "z".repeat(700)}),
        ))
        .collect();
