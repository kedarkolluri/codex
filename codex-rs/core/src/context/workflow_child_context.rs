...
pub(crate) fn bound_workflow_child_context_items(items: Vec<ResponseItem>) -> Vec<ResponseItem> {
...
fn bound_context_batch(items: Vec<ResponseItem>) -> Vec<ResponseItem> {
    ...
    if omitted {
        let marker: ResponseItem = ContextualUserFragment::into(WorkflowChildUserContext::new(
