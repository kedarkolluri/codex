...
async fn spawned_multi_agent_v2_child_migrates_legacy_hint_from_unloaded_parent() -> Result<()> {
    ...
    const UNRELATED_DEVELOPER_INSTRUCTION: &str =
    ...
    let mut builder = test_codex().with_config(|config| {
        ...
        config.multi_agent_v2.root_agent_usage_hint_text =
            Some(DRIFTED_ROOT_HINT.to_string());
    ...
    let (turn_result, unload_result) =
        tokio::join!(test.submit_turn(TURN_1_PROMPT), unload_parent);
    unload_result?;
    turn_result?;
