...
fn bounded_request_tool_names(request: &core_test_support::responses::ResponsesRequest) -> Vec<String> {
    ...
    let test = test_codex()
        ...
        .with_config(|config| {
            ...
            config.developer_instructions =
                Some("x".repeat(WORKFLOW_CUSTOM_CONTEXT_MAX_BYTES + 1));
        })
...
"#
    ...
    assert!(first_developer_groups
        .iter()
        .any(|group| group == std::slice::from_ref(&inherited_developer)));
    for request in child_requests {
        ...
        {
            assert_eq!(group.len(), 1, "workflow context must not aggregate text parts");
            assert!(
