...
impl InProcessCodeModeSession {
    ...
    /// Executes a Saved-workflow request with its exact caller-assigned cell identity.
    pub async fn execute_saved_workflow_with_cell_id(
        &self,
        cell_id: CellId,
        ...
        match request.output_policy {
            ExecuteOutputPolicy::Ordinary => {
                return Err("caller-assigned cell ID requires saved workflow output".to_string());
            }
