...
impl runtime::SessionRuntimeDelegate for ProtocolDelegate {
    ...
    async fn workflow_budget_snapshot(
        ...
        self.delegate
            .workflow_budget_snapshot(protocol_cell_id(&cell_id))
            .await
    }

    async fn journal_phase(&self, cell_id: runtime::CellId, title: String) -> Result<(), String> {
        self.delegate
            .journal_phase(protocol_cell_id(&cell_id), title)
            .await
    }

    async fn journal_log(&self, cell_id: runtime::CellId, message: String) -> Result<(), String> {
        self.delegate
    ...
    async fn replay_agent(
        ...
        // protocol crate. Unlike best-effort narration markers, replay accounting must be
        // acknowledged before the isolate receives the cached result.
        let entry = serde_json::to_value(&entry)
            .map_err(|error| format!("failed to serialize workflow replay entry: {error}"))?;
        self.delegate
            .replay_agent(
                protocol_cell_id(&cell_id),
                node_id,
    ...
    async fn workflow_progress(
        ...
        let _ = self
            .delegate
            .workflow_progress(protocol_cell_id(&cell_id), progress)
            .await;
    }
