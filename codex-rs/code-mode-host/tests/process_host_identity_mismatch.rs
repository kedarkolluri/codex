...
async fn wait_for_sidecar(path: &Path) {
    tokio::time::timeout(SIDECAR_TIMEOUT, async {
        loop {
            if path.try_exists().expect("inspect fixture sidecar") {
                break;
            }
    ...
    .await
    .expect("fixture sidecar did not appear");
}
...
async fn mismatched_workflow_id_recovers_on_a_fresh_spawned_host() {
    ...
    let mismatched_response_sent = sidecar_path(&host_program, ".mismatched-response-sent");
    ...
    wait_for_sidecar(&mismatched_response_sent).await;
    let recovered_response = tokio::time::timeout(FIXTURE_OPERATION_TIMEOUT, async {
        wait_for_sidecar(&observer_done).await;
        assert_eq!(
