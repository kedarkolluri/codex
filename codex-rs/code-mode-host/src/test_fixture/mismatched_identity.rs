...
pub(super) async fn run(current_exe: PathBuf, stdin_observer: bool) -> Result<()> {
    ...
    if stdin_observer {
        return run_stdin_observer(&post_mismatch_marker, &observer_armed, &observer_done).await;
    }
