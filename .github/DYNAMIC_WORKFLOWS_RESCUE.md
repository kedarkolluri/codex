...
- [Reviewable delivery stack](DYNAMIC_WORKFLOWS_DELIVERY_STACK.md)
...
The 2026-07-20 rescue re-audit closes the remaining release-blocking findings
in the endpoint/control/context scope: there are no open P0s or P1s in that
scope. Exact workflow ownership is sticky across local, in-memory, SQLite,
rollout, reconcile/backfill, and cold-resume paths; direct and cascading V1
collaboration operations reject exact workflow targets while preserving their
...
- the post-audit full crate suites: 95/95 thread-store, 169/169 state, and
  98/98 rollout;
- focused source-authority regressions: 1/1 state, 2/2 thread-store, 2/2
  rollout, and 2/2 core resume/completion-delivery;
- focused generic-boundary regressions: 5/5 final UUID/subtree, 6/6 persisted
...
The later security-hardening WIP also has an off-machine archival checkpoint:

- GitHub branch: `rescue/dynamic-workflows-wip-20260719`;
- commit: `97f03385584f8685fb6ef8dd798d6cc8a25f6d57`;
- tree: `37d0f9fb2ebd47eda71d58f364258eb24a6df0ef`;
- local complete-history bundle:
  `/home/kedar/projects/codex/dynamic-workflows-wip-20260719.bundle`;
- bundle SHA-256:
  `c2b868f7724ffedbe7f03f7daa865ff5aa8eb727bfa5053b9b5efabd1c6638a8`.

The remote ref, commit object, tree object, and bundle checksum were verified
...
Fresh read-only recon on 2026-07-19 found OpenAI `main` at
`678157acaa819d5510adfe359abb5d0392cfe461`, so the captured comparison above is
historical and cannot be used as the final rebase base. The local published
implementation head remains 42 commits ahead of fork `origin/main`; the
off-machine archival rescue checkpoint is 43 commits ahead. Both
`openai/codex#24721` and `openai/codex#25446` still appear in an explicit open
issue search. Recompute the complete divergence and conflict catalog in the
isolated reconstruction checkout after the next verified rescue checkpoint.
